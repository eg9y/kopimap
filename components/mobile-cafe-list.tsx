...
export const MobileCafeList: React.FC<CafeListProps> = ({
  ...
  const animateToHeight = (targetHeight: number) => {
    const currentHeight =
      (parseFloat(containerRef.current!.style.height) / window.innerHeight) *
      100;
  ...
  const bind = useDrag(
    ({ event, last }) => {
      ...
      const fingerPositionY =
        (event as TouchEvent).touches?.[0]?.clientY ||
        (event as MouseEvent).clientY;
      const newHeight =
        ((window.innerHeight - fingerPositionY) / window.innerHeight) * 100;
    ...
    },
    {
      ...
      filterTaps: true,
      bounds: {
        top: -window.innerHeight + SEARCH_BAR_HEIGHT,
        bottom: window.innerHeight - SEARCH_BAR_HEIGHT,
      },
      rubberband: true,
    },
  );
  ...
  return (
    <motion.div
      ...
      className="bg-slate-100 overflow-hidden z-[100] absolute bottom-0 left-0 right-0 rounded-t-lg shadow-lg"
    >
